...
class Qwen3TTSModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        logger.info(f"正在从 {pretrained_model_name_or_path} 加载模型...")
    ...
    def _load_audio_to_np(self, x: str) -> Tuple[np.ndarray, int]:
        if self._is_url(x):
            logger.info(f"正在从 URL 加载音频: {x}")
            with urllib.request.urlopen(x) as resp:
                audio_bytes = resp.read()
            with io.BytesIO(audio_bytes) as f:
                audio, sr = sf.read(f, dtype="float32", always_2d=False)
        elif self._is_probably_base64(x):
            logger.info("正在从 Base64 字符串加载音频")
            wav_bytes = self._decode_base64_to_wav_bytes(x)
            with io.BytesIO(wav_bytes) as f:
                audio, sr = sf.read(f, dtype="float32", always_2d=False)
        else:
            logger.info(f"正在从本地路径加载音频: {x}")
            audio, sr = librosa.load(x, sr=None, mono=True)

        if audio.ndim > 1:
            logger.info(f"音频为多声道，正在合并为单声道。原始维度: {audio.ndim}")
            audio = np.mean(audio, axis=-1)

        logger.info(f"音频加载完成。采样率: {sr}, 长度: {len(audio)}")
    ...
    def generate_voice_clone(
        ...
        **kwargs,
        ...
        logger.info(f"正在调用模型 generate 方法，Batch Size: {len(input_ids)}")
        ...
        # 处理 Voice Clone 模式下的 Token 切除
        # 核心模型生成的 talker_codes 往往包含了 Prompt 部分的对应 tokens，导致音频变长
        processed_codes = []
        for i, codes in enumerate(talker_codes_list):
            ...
            if ref_code_list is not None and ref_code_list[i] is not None:
                ...
                # 增加对额外残留的检测
                # 经验观察：在 ICL 模式下，生成序列开头的冗余部分可能比 ref_len 略长
                # 我们通过检查 codes 开头与 ref_item 的相关性来确定最佳切除点是不现实的，
                # 但可以基于用户反馈（多出1秒）来调整偏移量。
                # 考虑到 12Hz 模型中 12 tokens 约等于 1 秒。
                # 我们尝试寻找更准确的切除位置：如果在 ref_len 之后仍有明显的前导段，则扩大范围。
                cut_len = ref_len
                if codes.shape[0] > ref_len + 12:
                    # 如果多出的长度足够多，我们尝试多切除一部分（例如 1 秒引导音）
                    # 仅在长度远超预期时执行
                    logger.info(f"生成的序列较长 ({codes.shape[0]}), 将尝试加大切除力度以消除残留。")
                    cut_len = ref_len + 12 
                
                if codes.shape[0] > cut_len:
                    logger.info(f"正在切除前 {cut_len} 个 Token (含 Prompt {ref_len} 及安全偏移)")
                    processed_codes.append(codes[cut_len:])
                elif codes.shape[0] > ref_len:
                    logger.info(f"序列长度不足，仅切除 Prompt 长度 {ref_len}")
        ...
        wavs_out, fs = self.model.speech_tokenizer.decode([{"audio_codes": c} for c in processed_codes])
